...
###	plotting wind energy rose
	...
	total <- energy$total[num.sectors+1]
	unit <- attr(energy, "unit")
	
	if(num.classes>1) {
		e.cum <- energy[1:num.sectors,2:dim.data[2]]
		for(i in 2:num.classes) e.cum[,i] <- e.cum[,i] + e.cum[,i-1]
	} else {
		e.cum <- data.frame(energy[1:num.sectors,1])
		num.classes <- 1
	}
	...
	if(unit=="kWh/m^2/a") text(1, -1, expression(paste("kWh/m"^2,"/a")), pos=2, cex=cex-0.2, col="gray45")
	else text(1, -1, unit, pos=2, cex=cex-0.2, col="gray45")
	
	if(num.classes>1) {
		par(mar=c(0,0,0,0))
		plot(0, type="n", axes=FALSE, xlab="", ylab="")
		legend("left", legend=names(e.cum), title="Wind speed\n[m/s]", fill=col[1:num.classes], xjust=0, bty="n", cex=cex-0.2, x.intersp=0.4, y.intersp=0.8)
	}
}
