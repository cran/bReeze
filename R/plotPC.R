...
###	plotting power curve
	...
	if(!pc.list) unit <- attr(pc, "units")
	else unit <- attr(pc[[1]], "units")
	...
	if(!pc.list) {	
		...
		plot(pc$v[!is.na(pc$P)], pc$P[!is.na(pc$P)], type="l", xaxt="n", yaxt="n", xlab=xlab, ylab=ylab[1], col=col[1], lty=lty[1], lwd=lwd[1], cex=cex, xlim=xlim, ylim=ylim, col.axis=col.axis, col.lab=col.lab, bty="n")
		...
		if(cp && !is.null(pc$cp)) {
			...
			plot(pc$v[!is.na(pc$cp)], pc$cp[!is.na(pc$cp)], type="l", axes=FALSE, ylab="", xlab="", xlim=xlim, ylim=c(0,1), , col=col[2], lty=lty[2], lwd=lwd[2]) 
			axis(4, col=col.ticks, cex=cex, cex.axis=cex.axis, cex.lab=cex.lab, col.axis=col.axis, col.lab=col.lab)  
		}
		if(cp && !is.null(pc$cp) && ct && !is.null(pc$ct)) {
			par(new=TRUE, mgp=mgp, las=las)
			plot(pc$v[!is.na(pc$ct)], pc$ct[!is.na(pc$ct)], type="l", axes=FALSE, ylab="", xlab="", xlim=xlim, ylim=c(0,1), , col=col[3], lty=lty[3], lwd=lwd[3], cex=cex, cex.axis=cex, cex.lab=cex) 
		}
		if((!cp || is.null(pc$cp)) && ct && !is.null(pc$ct)) {
			par(new=TRUE, mgp=mgp, las=las)
			plot(pc$v[!is.na(pc$ct)], pc$ct[!is.na(pc$ct)], type="l", axes=FALSE, ylab="", xlab="", xlim=xlim, ylim=c(0,1), , col=col[2], lty=lty[2], lwd=lwd[2], cex=cex, cex.axis=cex, cex.lab=cex) 
			axis(4, col=col.ticks, cex=cex, cex.axis=cex.axis, cex.lab=cex.lab, col.axis=col.axis, col.lab=col.lab)  
		}
